...
由社群打造的自訂代理程式、提示詞與指令集合，讓您的 GitHub Copilot 體驗在不同領域、語言與應用場景中更上一層樓。

## 🚀 什麼是 Awesome GitHub Copilot？

本專案提供一套完整的工具組，透過以下專業資源強化您的 GitHub Copilot 體驗：

- **👉 [Awesome Agents](docs/README.agents.md)** - 專業的 GitHub Copilot 代理程式，整合 MCP 伺服器為特定工作流程與工具提供強化功能
- **👉 [Awesome Prompts](docs/README.prompts.md)** - 針對特定任務設計的提示詞，用於產生程式碼、文件與解決特定問題
- **👉 [Awesome Instructions](docs/README.instructions.md)** - 完整的程式碼規範與最佳實踐，適用於特定檔案模式或整個專案
- **👉 [Awesome Chat Modes](docs/README.chatmodes.md)** - 專業的 AI 角色與對話模式，適用於不同角色與情境
- **👉 [Awesome Collections](docs/README.collections.md)** - 精選的相關提示詞、指令與對話模式集合，依特定主題與工作流程分類組織

## 🌟 精選集合

探索我們依特定主題與工作流程精心整理的提示詞、指令與對話模式集合。

| 名稱 | 說明 | 項目 | 標籤 |
| ---- | ----------- | ----- | ---- |
| [Awesome Copilot](collections/awesome-copilot.md) | 元提示詞，協助您探索並產生精選的 GitHub Copilot 對話模式、集合、指令、提示詞與代理程式。 | 6 項 | github-copilot, discovery, meta, prompt-engineering, agents |
| [Partners](collections/partners.md) | GitHub 合作夥伴建立的自訂代理程式 | 17 項 | devops, security, database, cloud, infrastructure, observability, feature-flags, cicd, migration, performance |


## MCP Server

為了讓您輕鬆將這些客製化內容加入編輯器，我們建立了 [MCP Server](https://developer.microsoft.com/blog/announcing-awesome-copilot-mcp-server)，提供直接從本專案搜尋與安裝提示詞、指令與對話模式的功能。執行伺服器前，您需要先安裝並啟動 Docker。
...
自訂代理程式可在 Copilot 程式碼代理（CCA）、VS Code 與 Copilot CLI（即將推出）中使用。在 CCA 中，將議題指派給 Copilot 時，請從清單中選擇自訂代理程式。在 VS Code 中，您可以在代理程式工作階段中啟用自訂代理程式，與 Plan 和 Agent 等內建代理程式並用。
...
指令會根據檔案模式自動套用，並提供程式碼規範、框架與最佳實踐的情境指引。

### 💭 對話模式

啟用對話模式，取得針對特定角色（如架構師、DBA 或資安專家）量身打造的 AI 角色協助。

## 🎯 為什麼要使用 Awesome GitHub Copilot？

- **提升生產力**：預先建立的代理程式、提示詞與指令節省時間並提供一致的成果。
- **最佳實踐**：受益於社群精選的程式碼規範與模式。
- **專業協助**：透過專業的自訂代理程式取得專家級的指引。
- **持續學習**：隨時掌握各項技術的最新模式與實踐。

## 🤝 貢獻

我們歡迎您的貢獻！請參閱我們的[貢獻指南](CONTRIBUTING.md)，了解如何：
...
1. 遵循我們的檔案命名慣例與 frontmatter 需求
2. 徹底測試您的貢獻
3. 更新相應的 README 表格
4. 提交附有明確說明的 pull request

## 📖 專案結構

```plaintext
├── prompts/          # 特定任務的提示詞 (.prompt.md)
├── instructions/     # 程式碼規範與最佳實踐 (.instructions.md)
├── chatmodes/        # AI 角色與專業模式 (.chatmode.md)
...
- **支援**：查看我們的[支援指南](SUPPORT.md)以取得協助
- **行為準則**：我們遵循 [Contributor Covenant](CODE_OF_CONDUCT.md)

## ℹ️ 免責聲明

本專案中的客製化內容來源於第三方開發者的貢獻。GitHub 不驗證、背書或保證這些代理程式的功能或安全性。安裝前請仔細檢查任何代理程式及其文件，以了解其可能需要的權限與執行的操作。

---

**準備好提升您的程式開發體驗了嗎？**開始探索我們的[提示詞](docs/README.prompts.md)、[指令](docs/README.instructions.md)與[對話模式](docs/README.chatmodes.md)！

## Contributors ✨

感謝這些傑出的貢獻者（[emoji key](https://allcontributors.org/docs/en/emoji-key)）：
...
- [GitHub Copilot Chat 文件](https://code.visualstudio.com/docs/copilot/chat/copilot-chat) - 完整的對話功能指南
- [自訂對話模式](https://code.visualstudio.com/docs/copilot/chat/chat-modes) - 進階對話設定
- [VS Code 設定](https://code.visualstudio.com/docs/getstarted/settings) - 一般 VS Code 設定指南

## ™️ 商標

本專案可能包含專案、產品或服務的商標或標誌。Microsoft 商標或標誌的授權使用須遵守 [Microsoft 商標與品牌指南](https://www.microsoft.com/en-us/legal/intellectualproperty/trademarks/usage/general)。在本專案的修改版本中使用 Microsoft 商標或標誌時，不得造成混淆或暗示 Microsoft 的贊助。任何第三方商標或標誌的使用須遵守各該第三方的政策。
